...
| P0-09 | data-contract-checker | Replace Streamlit legacy/mismatched queries with one canonical shortlist read model | P0-03, P0-07 | Streamlit renders real fields for evaluated, deferred, verification and failed states | REVIEW | Uses `v_canonical_shortlist` (Migration 017) and removes legacy score fallbacks; deterministic Top Recommended; URL/content sanitization |
...
| ID | Owner | Assignment | Depends on | Acceptance criteria | Status | Notes |
|---|---|---|---|---|---|---|
| P1-01 | decision-policy-engineer | Implement `PASS / NEEDS_VERIFICATION / HARD_REJECT` with multiple codes, quotes and confidence | P0-03 | Workability rules are deterministic; an unknown workplace model is rejected after evidence review; vague non-workability language alone cannot hard reject | REVIEW | Two-axis prequalification active |
| P1-02 | decision-policy-engineer | Separate personal workability from domain/career value | P1-01 | Hard workability conflicts are non-compensable; workplace-model rejection is distinct from other verification evidence | DONE | Workability facts separated from lane scoring |
| P1-03 | decision-policy-engineer | Load lane definitions and thresholds from YAML; support secondary/unclassified lanes | P0-03 | Six lane configurations load; title-only counterfactuals are stable; zero/random embeddings cannot default to core AI | REVIEW | Lane thresholds calibrated (0.35–0.40) |
| P1-04 | decision-policy-engineer | Replace budget-cap rejection with durable deferral and fair per-lane selection | P0-04, P1-03 | Eligible jobs are selected within a durable per-run lane budget; overflow records `DEFERRED_BUDGET` and remains eligible for a later run | DONE | Budget runs, usage, queue identity and deferral evidence are persisted |
...
| P1-06 | ingestion-source-engineer | Implement six outbound discovery scouts as query/source planners | P1-05 | Scouts cover each configured lane and only produce queries/watchlists or ingest verified postings; no fabricated jobs | IN_PROGRESS | Verified active boards running via SourceBroker |
