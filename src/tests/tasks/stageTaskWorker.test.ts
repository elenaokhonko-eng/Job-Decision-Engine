...
describe("stageTaskWorker", () => {
  ...
  it("versions profile matching task keys by active profile version", () => {
    ...
    expect(
      buildPipelineTaskKey("EXTRACT_DETERMINISTIC_REQUIREMENTS", "version-1", "deterministic_v3", undefined, "repair")
    ).toBe("EXTRACT_DETERMINISTIC_REQUIREMENTS:version-1:deterministic_v3:repair");
    expect(
    ...
    expect(
      calls.some(
        (call) =>
          ...
          call.params?.[2] === "EXTRACT_DETERMINISTIC_REQUIREMENTS:version-1:deterministic_v3"
      )
    ).toBe(true);
  });
          ...
          rows: [
            {
              ...
              task_key: "EXTRACT_DETERMINISTIC_REQUIREMENTS:version-empty:deterministic_v3",
