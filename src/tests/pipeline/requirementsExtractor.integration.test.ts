...
describe.skipIf(skipReal)('Requirements extraction integration (temporary schema)', () => {
  it('persists requirement rows, extraction runs, and stage events without duplicates on rerun', async () => {
    ...
    try {
      ...
      const sparseTaskKey = `EXTRACT_DETERMINISTIC_REQUIREMENTS:${sparseJobVersionId}:deterministic_v3`;
      ...
      const repairTask = await client.query(
        `SELECT payload
         FROM pipeline_tasks
         WHERE task_key = $1`,
        [`EXTRACT_DETERMINISTIC_REQUIREMENTS:${lateJobVersionId}:deterministic_v3:repair`]
      );
